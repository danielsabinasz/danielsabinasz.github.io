...
var brain3d = (function() {
    ...
    // Base URL for model and cerebropedia assets, hosted on Cloudflare R2.
    var ASSET_BASE_URL = "https://pub-d2221051c06243a1bf12c7cf36c05aad.r2.dev/";
    ...
    function loadModels() {
        ...
        loadModel(ASSET_BASE_URL + "models/mni_brain.vtk", vtkLoader);
    ...
    function createBrain() {
        for (var view in views) {
            brain_grey_meshes[view] = new THREE.Mesh( geometries[ASSET_BASE_URL + "models/mni_brain.vtk"], materials["grey"] );
	...
	setShowGrayMatter: function(show) {
        ...
        loadSliceImage: function(axis, coordinate) {
            var path = ASSET_BASE_URL + "cerebropedia/slices/" + mni_image + "_" + axis + "_" + Math.round(coordinate) + ".png";
        ...
        updateSliceRegionImage: function(view, axis, side, coordinate) {
            ...
            for (var i = 0; i < active_nodes.length; i++) {
                ...
                var path = ASSET_BASE_URL + "cerebropedia/regions/" + node.id + "_" + axis + "_" + Math.round(coordinate) + ".png";

                var region_image = new Image();
                // Required so the cross-origin (R2) image can be drawn to a canvas
                // and read back via getImageData() without tainting it.
                region_image.crossOrigin = "anonymous";
        ...
        addNode: function(node, color) {
            ...
            if (typeof(node.meshes) == "undefined") {
                ...
                console.log("Load: " + ASSET_BASE_URL + "cerebropedia/3d/" + node.id + ".stl");
                stlLoader.load(ASSET_BASE_URL + "cerebropedia/3d/" + node.id + ".stl", function (geometry) {
        ...
        saveImage: function(view) {
            ...
            $("#modal .modal-body").html("<div style=''><p>Right-click the image and click on 'save image as'.</p><p>Refer to the source as follows:<br><pre>Image source: <a href='" + window.location.href + "' target='blank'>" + window.location.href + "</a><br>Based on an MRI brain template from Fonov et al. (2009, 2011) and the Harvard-Oxford Atlas (Desikan et al., 2006; Frazier et al., 2005; Goldstein et al., 2007; Makris et al., 2006; https://cma.mgh.harvard.edu; https://fsl.fmrib.ox.ac.uk/fsl/fslwiki/Atlases).</pre></p><p>Then add this to your references:<br><pre>Desikan RS, Ségonne F, Fischl B, Quinn BT, Dickerson BC, Blacker D, Buckner RL, Dale AM, Maguire RP, Hyman BT, Albert MS, Killiany RJ. An automated labeling system for subdividing the human cerebral cortex on MRI scans into gyral based regions of interest. Neuroimage. 2006 Jul 1;31(3):968-80.<br>Fonov, V, Evans, AC, Botteron, K, Almli, CR, McKinstry, RC, Collins, DL, & Brain Development Cooperative Group. (2011). Unbiased average age-appropriate atlases for pediatric studies. Neuroimage, 54(1), 313-327.<br>Fonov, VS., Evans, AC, McKinstry, RC, Almli, CR, & Collins, DL (2009). Unbiased nonlinear average age-appropriate brain templates from birth to adulthood. Neuroimage, 47(1), S102.<br>Frazier JA, Chiu S, Breeze JL, Makris N, Lange N, Kennedy DN, Herbert MR, Bent EK, Koneru VK, Dieterich ME, Hodge SM, Rauch SL, Grant PE, Cohen BM, Seidman LJ, Caviness VS, Biederman J. Structural brain magnetic resonance imaging of limbic and thalamic volumes in pediatric bipolar disorder. Am J Psychiatry. 2005 Jul;162(7):1256-65.<br>Goldstein JM, Seidman LJ, Makris N, Ahern T, O'Brien LM, Caviness VS Jr, Kennedy DN, Faraone SV, Tsuang MT. Hypothalamic abnormalities in schizophrenia: sex effects and genetic vulnerability. Biol Psychiatry. 2007 Apr 15;61(8):935-45 Makris N, Goldstein JM, Kennedy D, Hodge SM, Caviness VS, Faraone SV, Tsuang MT, Seidman LJ. Decreased volume of left and total anterior insular lobule in schizophrenia. Schizophr Res. 2006 Apr;83(2-3):155-71.<br>Makris N, Goldstein JM, Kennedy D, Hodge SM, Caviness VS, Faraone SV, Tsuang MT, Seidman LJ. Decreased volume of left and total anterior insular lobule in schizophrenia. Schizophr Res. 2006 Apr;83(2-3):155-71</pre></p><img src='" + dataURL + "' style='width: 70%; margin: 0px auto;'></div>");
//$("#modal").dialog({width: 800, autoOpen: false, modal: true});
//$("#modal").dialog("open");
$("#modal").modal("show");
	},
